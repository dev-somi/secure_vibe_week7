...
export default function VulnerabilityListPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const severityCounts = useMemo(() => {
    ...
    results.forEach(v => {
      const sev = v.extra.severity.toLowerCase()
  ...
  const filteredVulnerabilities = useMemo(() => {
    return results.filter(v => {
      ...
      if (sev === 'error' || sev === 'critical') mappedSeverity = 'critical'
      else if (sev === 'warning' || sev === 'high') mappedSeverity = 'high'
      ...
      const matchesSearch = 
        ...
        v.extra.message.toLowerCase().includes(searchQuery.toLowerCase())
      
      return matchesSeverity && matchesSearch
    })
  }, [results, selectedSeverity, searchQuery])
        ...
        <section className="flex-1 flex flex-col min-w-0">
          ...
          <div className="space-y-4">
            {filteredVulnerabilities.map((vuln, index) => {
              ...
              const title = vuln.check_id.split('.').pop() || 'Unknown Issue'
                  ...
                  <div className="p-5 flex flex-col">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-sm ${getSeverityBadgeColor(mappedSeverity)}`}>
